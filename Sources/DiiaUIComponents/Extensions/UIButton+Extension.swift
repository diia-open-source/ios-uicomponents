...
public extension UIButton {
    ...
    func addSpacingBetweenTextAndImage(spacing: CGFloat) {
        let inset = spacing / 2
        let isImagePositionRight = semanticContentAttribute == .forceRightToLeft
        
        imageEdgeInsets = .init(top: .zero,
                                left: isImagePositionRight ? inset : -inset,
                                bottom: .zero,
                                right: isImagePositionRight ? -inset : inset)
        titleEdgeInsets = .init(top: .zero,
                                left: isImagePositionRight ? -inset : inset,
                                bottom: .zero,
                                right: isImagePositionRight ? inset : -inset)
        contentEdgeInsets = .init(top: .zero, left: inset, bottom: .zero, right: inset)
    }
}
